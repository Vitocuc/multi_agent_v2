...
_ADMIN_ROLE = "concessionaire_admin"
...
def get_admin_user_id(
    ...
    """Resolve admin identity from session cookie.

    Requires role == "concessionaire_admin" in the JWT payload.
    Returns the admin user UUID. Raises 401/403 otherwise.
    Admin routes are intentionally on a separate dependency — user JWTs
    with no role claim cannot satisfy this check (403, not 401).
    """
    if not session_token:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="not_authenticated")
